	...
	try {
		...
		if (selectedCalendarIds.length === 0) {
			return NextResponse.json({
				...
				externalCalendarCount: 0,
				conflicts: [],
				message: "No non-DueForge calendars selected for reconciliation.",
			});
		}
		...
		const conflicts = links
			.map((link) => {
				...
				return {
					...
					severity,
					recommendation: severity === "high" ? "Reschedule immediately or shorten this block." : "Review and adjust this block if needed.",
				};
			})
		...
		await prisma.activityEvent.create({
			data: {
				...
				entityType: "schedule",
				entityId: user.id,
				eventType: "schedule.reconciled",
				payloadJson: {
					checkedCount: links.length,
					conflictCount: conflicts.length,
					externalCalendarCount: selectedCalendarIds.length,
				},
			},
		});

		logTelemetryEvent(telemetryEvents.SCHEDULE_RECONCILED, {
			userId: user.id,
			checkedCount: links.length,
			conflictCount: conflicts.length,
			externalCalendarCount: selectedCalendarIds.length,
